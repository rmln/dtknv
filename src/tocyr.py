...
class ToCyr:
    """Converts textual content files form Cyrillic to Latin script.
    """
    ...
    PATHIN = None
    PATHOUT = None
    #DIRTMP = r'..\_tmp' # For debugging only.
    ...
    def _convertfile(self, f):
        ...
        if self.extension in self.TEXTFILES:
            ...
            # Rename media files in word/media for DOCX
            # This seemed to be unnecessary...
            #self._renameresfiles(os.path.join(self.unzipped, 'word', 'media'))
            self._zip(outpath)
        #Percentage counter
    ...
    def _convertdir(self):
        ...
        # Calculate size of all files for conversion:
    ...
    def _converfname(self, path):
        """Sees if filename/dirname needs conversion, and convert if yes."""
    ...
    def _unzip(self, f):
        ...
        # This has to  be done here, or the program would not find the path.
    ...
    def _zip(self, path):
        """Zip file which the path ponts to."""
        if self.conversiontype == 'files':
